...
manual` is selected. In the Editor, each API uses an iOS-inspired Game view preview
with white surfaces and a dark pill-shaped Toast. Final native appearance is verified
on a device; the PlayMode tests cover
scene loading, UI wiring, viewport ratio, and result updates.
