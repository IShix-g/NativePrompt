...
3. Confirm there are zero failed tests and specifically check coverage for Loading
   ...
   newest-options restoration, reset cleanup, and normalized strategy input. Also
   confirm `LoadingStarted` / `LoadingEnded` ordering, metadata, active counts,
   all four end reasons, no duplicate event on restored options, and no events when
   strategy startup fails.
4. In PlayMode, confirm the sample scene loads, every Loading button is bound, the
   centered 9:16 viewport remains in bounds, and the result label shows Loading
   start/end lifecycle events with metadata and active counts.
...
Unity Test Runner execution is user verification and is not performed by the AI agent.

### iOS and Android device procedure (user verification)

Physical-device builds and checks can take substantial time and are performed by the user.
...
The `LATEST RESULT` panel displays the most recent `LoadingStarted` or `LoadingEnded`
event with a shortened request ID, tag, group, end reason, and `ActiveCount`. The
device log uses the `[NativePrompt Sample]` prefix and includes the full request ID.
Selecting a Loading option for the first time should report `LoadingStarted` with
`active=1`. Changing an option while it is visible should report the new start with
`active=2`, followed by the replaced request ending with `active=1`. `Dismiss loading`
or the three-second automatic dismissal should end with `Dismissed` and `active=0`.
